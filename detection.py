...
class Detection(QThread):
    ...
    def _annotate_frame(self, frame, boxes, confs):
    ...
    def run_webcam_or_rtsp(self):
        ...
        while self.running:
            ...
            if self.model is not None and frame_count % 2 == 0:
                try:
                    results = self.model(frame, imgsz=480, conf=0.40, device='cpu', verbose=False)
                    prediction = results[0]
                    boxes = prediction.boxes.xyxy.cpu().numpy() if hasattr(prediction.boxes, 'xyxy') else []
                    confs = prediction.boxes.conf.cpu().numpy() if hasattr(prediction.boxes, 'conf') else []
                except Exception as e:
                    print(f"Error during inference: {e}")
                    boxes, confs = [], []

            annotated_frame = self._annotate_frame(frame, boxes, confs)

            # Save frame if confidence > 50%
            self._save_high_confidence_frame(annotated_frame, confs)
